  ...
  try {
    ...
    if (parts && Array.isArray(parts)) {
      for (const item of parts) {
        ...
        const unitBasePrice = Number(item.price) || 0;
        const quantity = Number(item.quantity) || 1;
        const pGstPct = item.gstPercentage !== undefined && item.gstPercentage !== null && item.gstPercentage !== '' ? Number(item.gstPercentage) : partsGstPct;
    ...
    if (customItems && Array.isArray(customItems)) {
      for (const item of customItems) {
        ...
        const cGstPct = item.gstPercentage !== undefined && item.gstPercentage !== null && item.gstPercentage !== '' ? Number(item.gstPercentage) : partsGstPct;
