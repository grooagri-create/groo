...
const Blogs = () => {
  ...
  const fetchBlogs = async () => {
    try {
      setLoading(true);
      const res = await websiteService.getBlogs();
      if (res.data.success) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const data = { ...form, tags: form.tags.split(",").map(t => t.trim()).filter(t => t) };
    try {
      if (editingId) {
        const res = await websiteService.updateBlog(editingId, data);
        if (res.data.success) {
        ...
        const res = await websiteService.createBlog(data);
        if (res.data.success) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await websiteService.deleteBlog(id);
      if (res.data.success) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? "Edit Blog" : "Add Blog"}>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input type="text" className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-primary-500 outline-none" placeholder="Ex: farming, organic, harvest" value={form.tags} onChange={e => setForm({...form, tags: e.target.value})} />
          </div>
          <div className="flex items-center gap-2">
            <input type="checkbox" id="blog-active" checked={form.isActive} onChange={e => setForm({...form, isActive: e.target.checked})} />
            <label htmlFor="blog-active" className="text-sm font-semibold">Show on website</label>
          </div>
