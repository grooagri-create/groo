...
const TopEquipment = ({ bookings = [], periodLabel = 'Top Equipment' }) => {
  ...
  const topItems = useMemo(() => {
    const map = new Map();
    bookings.forEach((b) => {
      const name = b.serviceType || 'Unknown Equipment';
  ...
  const totalPages = Math.ceil(topItems.length / itemsPerPage) || 1;
  const paginated = useMemo(() => {
    const start = (currentPage - 1) * itemsPerPage;
    return topItems.slice(start, start + itemsPerPage);
  }, [topItems, currentPage]);
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
      ...
      <div className="space-y-4">
        ...
        {topItems.length === 0 && <p className="text-gray-500 text-sm">No data found.</p>}
      </div>
